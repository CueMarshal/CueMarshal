...
import_source() {
    ...
    if [ ! -d "/source" ]; then
        echo "  /source directory not found (Kubernetes deployment) — skipping source import"
        echo "  Push source manually: git remote add gitea http://<host>/${ORG_NAME}/cuemarshal.git && git push"
        return 0
    fi
...
seed_labels_webhook_runner() {
    ...
    if [ -n "${REG_TOKEN}" ]; then
        ...
        # Persist runner token to Kubernetes secret so the runner pod can read it
        ...
        if [ -n "${SA_TOKEN}" ] && [ -n "${NAMESPACE}" ]; then
            REG_TOKEN_B64=$(echo -n "${REG_TOKEN}" | base64 | tr -d '\n')
            curl -sf -X PATCH \
                -H "Authorization: Bearer ${SA_TOKEN}" \
                -H "Content-Type: application/strategic-merge-patch+json" \
                --cacert /var/run/secrets/kubernetes.io/serviceaccount/ca.crt \
                "https://kubernetes.default.svc/api/v1/namespaces/${NAMESPACE}/secrets/${K8S_SECRET_NAME}" \
                -d "{\"data\":{\"runner-registration-token\":\"${REG_TOKEN_B64}\"}}" > /dev/null 2>&1 \
                && echo "  Runner token persisted to Kubernetes secret" \
                || echo "  WARNING: Could not persist runner token to secret"
        fi
    else
