...
docker build -t ${REGISTRY}/conductor:${TAG} -t ${REGISTRY}/conductor:latest "$PROJECT_ROOT/services/conductor"
load_to_cluster "${REGISTRY}/conductor:${TAG}"

# Build gateway
echo "Building gateway..."
docker build -t ${REGISTRY}/gateway:${TAG} -t ${REGISTRY}/gateway:latest "$PROJECT_ROOT/services/gateway"
load_to_cluster "${REGISTRY}/gateway:${TAG}"

# Build landing
echo "Building landing..."
docker build -t ${REGISTRY}/landing:${TAG} -t ${REGISTRY}/landing:latest "$PROJECT_ROOT/services/landing"
...
docker build -t ${REGISTRY}/runner:${TAG} -t ${REGISTRY}/runner:latest -f "$PROJECT_ROOT/services/runner/Dockerfile" "$PROJECT_ROOT/services"
load_to_cluster "${REGISTRY}/runner:${TAG}"

# Build MCP servers (all require project root context for shared dependencies)
echo "Building MCP servers..."
docker build -t ${REGISTRY}/mcp-gitea:${TAG} -t ${REGISTRY}/mcp-gitea:latest -f "$PROJECT_ROOT/services/mcp-servers/gitea-mcp/Dockerfile" "$PROJECT_ROOT/services/mcp-servers"
load_to_cluster "${REGISTRY}/mcp-gitea:${TAG}"

docker build -t ${REGISTRY}/mcp-conductor:${TAG} -t ${REGISTRY}/mcp-conductor:latest -f "$PROJECT_ROOT/services/mcp-servers/conductor-mcp/Dockerfile" "$PROJECT_ROOT/services/mcp-servers"
load_to_cluster "${REGISTRY}/mcp-conductor:${TAG}"

docker build -t ${REGISTRY}/mcp-system:${TAG} -t ${REGISTRY}/mcp-system:latest -f "$PROJECT_ROOT/services/mcp-servers/system-mcp/Dockerfile" "$PROJECT_ROOT/services/mcp-servers"
load_to_cluster "${REGISTRY}/mcp-system:${TAG}"

docker build -t ${REGISTRY}/mcp-vector:${TAG} -t ${REGISTRY}/mcp-vector:latest -f "$PROJECT_ROOT/services/mcp-servers/vector-mcp/Dockerfile" "$PROJECT_ROOT/services/mcp-servers"
load_to_cluster "${REGISTRY}/mcp-vector:${TAG}"

docker build -t ${REGISTRY}/mcp-sonar:${TAG} -t ${REGISTRY}/mcp-sonar:latest -f "$PROJECT_ROOT/services/mcp-servers/sonar-mcp/Dockerfile" "$PROJECT_ROOT/services/mcp-servers"
