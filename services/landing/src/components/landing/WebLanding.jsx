...
import { Bot, Users, LogOut, User as UserIcon, Sparkles, Menu, Kanban, Moon, Sun } from 'lucide-react';
...
export default function WebLanding() {
  ...
  const loadSessions = async () => {
    try {
      ...
      const cached = await storage.getSessionsList();
      if (cached.length > 0 && sessions.length === 0) {
        setSessions(cached);
      }
      const data = await fetchChatSessions();
      const fresh = data.sessions || [];
      setSessions(fresh);
      await storage.saveSessionsList(fresh);
    } catch (err) {
  ...
  const shortcuts = useMemo(() => ({
    'mod+/': () => setSidebarOpen(prev => !prev),
    'mod+k': () => {
      setSidebarOpen(true);
      setTimeout(() => {
        document.querySelector('[data-sidebar-search]')?.focus();
      }, 100);
    },
    'mod+shift+n': () => handleNewChat(),
  }), []);
  ...
  return (
    <div className="h-dvh bg-cuemarshal-white dark:bg-dark-bg text-cuemarshal-charcoal dark:text-dark-text font-inter flex flex-col overflow-hidden transition-colors">
      ...
      <div className="flex flex-1 flex-col lg:flex-row overflow-hidden">
        {/* Desktop Sidebar */}
        <div className="hidden md:flex w-[240px] xl:w-[280px] shrink-0 border-r border-gray-200 dark:border-dark-border">
          <SessionSidebar
            sessions={sessions}
        ...
        <div className="lg:hidden flex p-1.5 sm:p-2 bg-gray-50 dark:bg-dark-surface border-b border-gray-200 dark:border-dark-border gap-1.5 sm:gap-2 shrink-0">
          {[
            { id: 'chat', icon: Bot, label: 'Chat' },
            { id: 'projects', icon: Kanban, label: 'Projects' },
            { id: 'activity', icon: Users, label: 'Team' },
            ...
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg font-medium text-sm transition-colors flex justify-center items-center gap-2 ${
                activeTab === id
                  ? 'bg-white dark:bg-dark-card shadow-sm text-cuemarshal-navy dark:text-white border border-gray-200 dark:border-dark-border'
        ...
        <div className={`flex-1 flex flex-col bg-white dark:bg-dark-bg min-w-0 ${activeTab !== 'chat' ? 'hidden lg:flex' : 'flex'}`}>
          <ChatView
            currentSessionId={currentSessionId}
        ...
        {/* Projects Panel -- full width on mobile, hidden on desktop (shown via tab) */}
        <div className={`flex-1 flex flex-col ${activeTab === 'projects' ? 'flex lg:hidden' : 'hidden'}`}>
          <ProjectBoard />
        </div>

        {/* Activity Panel -- fixed width on desktop, full width on mobile/tablet */}
        <div className={`flex-1 lg:flex-none lg:w-[380px] xl:w-[420px] flex flex-col bg-cuemarshal-grey dark:bg-dark-surface lg:border-l lg:border-gray-200 dark:lg:border-dark-border ${activeTab === 'activity' ? 'flex' : 'hidden lg:flex'}`}>
          <ActivityView />
        </div>
      </div>
    </div>
